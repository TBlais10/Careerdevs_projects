...
//let departments = ["d001", "Marketing"],

//this array represents the department ids and their names
//index 0 is the department id#
//index 1 is the name of the department



//let employeeDepartment = [10001, "d005", "1986-06-26", "9999-01-01"],

//this array represents the employee, the department they work in and their start/end date
//index 0 is the employee id #
//index 1 is the department id #
//index 2 is the start date
//index 3 is the end date
//some have swapped departments (lines 26 + 27, 35 + 36, and 47 + 48)


// employeeSalaries =  [10035, 61793, "1997-09-03", "1998-09-03"],

//this array contains information of each employee's salary changes
//index 0 is the employee id#
//index 1 is the salary of that employee (at that time)
//index 2 is the starting date for the salary referenced on index 1
//index 3 is the end date for the salary mentioned on index 1



//let employees = [10122, "1965-01-19", "Ohad", "Esposito", "M", "1992-12-23"],

// index 0 is the employee id number
// index 1 is that employee's birthday
// index 2 is the first name of the employee
// index 3 is the last name of the employee
// index 4 is the gender of the employee
// index 5 is the hiring date of the employee

//PATTERNS!
//The department # is referenced  inside of the employee departments array
//The employee id # is referenced in nearly every array.
//the best reference for the employee id # is the employee array
//the salary array has varying salary increses and decreces
//the salary array keeps track of every time that employee has had a change in pay
//the date 9999-01-01 means the employee is still working there
//the first employee id # is always 10001
//salary numbers is a number data type
//any dates are a string data type
//index 0 between all arrays share similar data - the employee id
...
//class notes pt 2 (tuesday)
//throw or loader.js -> not your code.
//vs code has es lint = auto completes your code for you.
//auto import -> recognizes the need for the code and tries to import. You can also import code.
...
//console.log(departments) //does not have more than 10 sub arrays. Does not need the for loop

// function companyInfo(array, num) {
//     let firstTenSubs = []
//     for (let i = 0; i < num; i++) {
//         if (array.length < num) {

//             firstTenSubs.push(array[i])
...
//     }
//      console.log(firstTenSubs)

// }

// companyInfo(departments, 10)
// companyInfo(employeeDepartment, 10)
// companyInfo(employees, 10)
// companyInfo(employeeSalaries, 10)


//first attempt at console logging the code
//for (let i = 0; i < 10; i++) {
...
//challenge 1c
//log all employee names and their department names
//grab the first 40

// function whereTheyWork(arr) {
//     let id = [] //for the .include() to parse out the repeats?
...
//                 if (arr[i][0] == employeeDepartments[j][0] && employeeeDepartments[j][1] == dep[k][0]) {

//                     //id.push(arr[i][0]) //testing to check for repeated id#s
//                     //pushed too many ids due to position w/in for loop

//                     //console.log(id)
//                    console.log(`${arr[i][2]} ${arr[i][3]} id # ${employeeDepartments[j][0]} works in the ${departments[k][1]} department.\n`)
...
//i see if statements being used to find the right info

//spudo code

//if the salary is grater than 60k, then we need to out put the employee name, department, and dept number.
//the info for the employee data repeats because of the changes. 
//when we get the employee number, we then need to get the infomation to use that to pass thru the other arrays

let employeeId = [] //this needs to contain the the id numbers of the employees who make over 60k
let over60k = []  //sorts the salaries over 60k
let biggestSalary = [] //sorts the biggest salary among multiple salaries for one employee

//time w/ gabe june 23rd
//the data bridge is the employeeDepartments = employee id and department id are found there

//formulas for finding info
//formula for finding employee id
//return an index number
//10001 + x = 0
//10057 + x = 56
//what needs to be written for x so these to be true?
//the answer is x = -10001


//this way should eliminate the use of a few for loops, they will still be needed in the code.

function idToEmpIndex(employeeID) {//only works on the employees arr
...
//idToEmpIndex(10010)
// this idea also applies to departmets

function idToDepIndex(departmentID) {//this only works with the departments arr

    return departmentID.slice(-3) - 1

}

//idToDepIndex(d001)

// let randomNum = Math.floor(Math.random() * employeeDepartments.length -1) //grabbing a random number

// //firstName is in departement ___

// let randomData = employeeDepartments[randomNum]

let empId = 100035 

let deptId = 'd004'

let employeeIndex = idToEmpIndex(empId)
let departmentIndex = idToDepIndex(deptId)

let emplData = employees[ employeeIndex ]
let deptData = departments[ departmentIndex ]

console.log(emplData)
console.log(deptId)

//trying to use the functions inside of for loops for solving challenge 2a

for (let i = 0; i < employeeSalaries.length; i++) {
    if (employeeSalaries[i][1] > 60000 && !employeeSalaries[i][0].includes(employeeSalaries[i][0])) {
        for (let j = 0; j < employeeDepartments.length; j++) {
            if (emplData)
        }
    }
...
// for (let i = 0; i < employeeDepartments.length; i++) { //we are making the report for the employees
...
//         if (employeeSalaries[j][1] > 60000 && !employeeSalaries[j][0].includes(employeeSalries[j][0])) {
...
// function test(sal, emp, empDep, dep) {

//     for (let i = 0; i < sal.length; i++) {

//         for (let j = 0; j < emp.length; j++) {

//             for (let k = 0; k < empDep.length; k++) {

//                 for (let l = 0; l < dep.length; l++) {

//                     if (sal[i][1] > 60000 && !sal.includes(sal[i][0] && sal[i][0] == emp[j][0] && emp[j][0] == empDep[k][0] && empDep[k][1] == dep[l][0])) { //is the issue I have too many if statements on one line?
//                         //if so, how do I break it up in a way I get everything

//                         //console.log(sal[i][1])
//                         //return console.log('This Works')


//                         console.log(`${emp[j][2]} ${emp[j][3]}, employee ID# ${empDep[k][1]} works in ${dep[l][1]} and makes $${sal[i][1].toLocaleString()} `)
//                         //log needs all 4 loops to show in terminal w/ no error
//                         //why did 1c have no problems when being 3 deep?


//                         //PROBLEMS!!
//                         //out putting several different dep id + dep names
//                         //the salary is the same due to the loop depth
//                         //names repeating numerous times.
//                     }
...
// test(employeeSalaries, employees, employeeDepartments, departments)


//new code w/ different if statements

// function test(sal, emp, empDep, dep) {

//     for (let i = 0; i < sal.length; i++) {

//         if (sal[i][1] > 60000 && !sal.includes(sal[i][0] && sal[i][0]) {
//             for (let j = 0; j < emp.length; j++) {

//                 for (let k = 0; k < empDep.length; k++) {

//                     if (sal[i][0] == emp[j][0] && emp[j][0] == empDep[k][0]) {

//                         for (let l = 0; l < dep.length; l++) {

//                             if (emp[j][0] == empDep[k][0] && empDep[k][1] == dep[l][0]) {
...
//         if (arr[i][0] == id[j]) { //checking employees array for matching id #
//             statement.push(`${arr[i][2]} ${arr[i][3]}`) //pushes first and last name into the statement array

//             for (let k = 0; k < empDep.length; k++) {

//                 if (id[j] == empDep[k][0]) { //checks the id # against employeeDepartment arr
//                     departmentId.push(empDep[k][1]) //pushes department id # into departmentId arr
//                 }
...
//                 if (dep[l][0] == departmentId[j])//checks departmentId arr for matching ids in departments arr.
//                     departmentNames.push(dep[1][1]) // finds the department name
//                 //too many for loops. must be a better way
//             }
//         }
